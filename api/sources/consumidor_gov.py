...
import pandas as pd
from curl_cffi import requests  # Bypass WAF/JA3

# --- CONFIGURAÇÕES ---
TIMEOUT = int(os.getenv("CG_TIMEOUT", "180")) # Timeout maior para dumps grandes
...
class Agg:
    ...
    score_sum: float = 0.0
    resolved_claims: int = 0 # Contagem de "Resolvida"
    ...
    def merge_raw(self, raw: dict) -> None:
        ...
        # Reconstrói somas para média ponderada
        avg_sat = float(stats.get("overallSatisfaction", 0.0))
        if ec > 0:
            self.score_sum += avg_sat * ec
            
        # Índice de Solução (se disponível)
        idx_sol = float(stats.get("solutionIndex", 0.0)) # 0.0 a 1.0 ou 0 a 100
        # Normaliza se vier como porcentagem média
        # Como não temos o count de avaliações de solução separado no raw simples,
        # assumimos que 'evaluatedCount' serve de base ou usamos regra de 3 simples.
        # Melhor abordagem incremental: somar contadores absolutos se tivessem sido salvos.
        # Se o raw não tem 'resolvedCount', tentamos inferir.
        # Para simplificar e não quebrar: acumulamos 'resolved_claims' se disponível no raw.
        rc = stats.get("resolvedCount")
        if rc is not None:
            self.resolved_claims += int(rc)
        else:
            # Fallback: tenta reverter do índice
            self.resolved_claims += int((idx_sol if idx_sol <= 1.0 else idx_sol/100.0) * tc)

        # Prioriza CNPJ existente
    ...
    def to_public(self) -> dict:
        ...
        return {
            ...
            # Atalho compatível com intelligence.py
            "indexes": {
                "b": {"nota": avg_sat}
            }
        }
...
# --- CAMADA DE TRANSPORTE (Download & Detecção) ---
...
def _get_latest_dump_url(client: requests.Session) -> Optional[str]:
    ...
    try:
        ...
        for h in hrefs:
            ...
            # Heurística: Ano atual/recente
            score = 0
            # Extrai ano do link ou assume atual
            matches = re.findall(r"(20\d{2})", full)
            year = int(matches[0]) if matches else 0
            score += year * 10
            if "csv" in full.lower(): score += 5
            if "zip" in full.lower(): score += 3
...
def download_dump_to_file(url: str, client: requests.Session) -> Optional[Path]:
    """Baixa stream para arquivo temporário (evita RAM explode)."""
...
def open_dump_file(path: Path) -> io.BytesIO:
    """
    Identifica formato (GZ, ZIP, CSV) e retorna handle de leitura para o CSV.
    Retorna BytesIO ou file object.
    """
    ...
    if sig.startswith(b"\x1f\x8b"):
        ...
        # Abre o GZ e retorna o stream
        return gzip.open(path, "rb")
        
    # ZIP (PK 03 04)
    elif sig.startswith(b"PK\x03\x04"):
        print("CG: Formato ZIP detectado.")
        # Precisamos extrair o CSV. Como ZipFile precisa de seekable, 
        # e gzip stream não é, melhor extrair o maior arquivo para temp.
        with zipfile.ZipFile(path, "r") as z:
            ...
            return z.open(target) # Retorna stream do arquivo dentro do zip
            
    else:
        print("CG: Assumindo CSV direto.")
        return open(path, "rb")

# --- PROCESSAMENTO E FATIAMENTO ---
...
def pick_columns(cols: list[str]) -> Tuple[Any, Any, Any, Any, Any]:
    ...
    c_resolved = find(["resolvida", "situação", "status"]) # Busca coluna S/N
...
def process_dump_to_monthly(dump_path: Path, target_yms: List[str], output_dir: str):
    """
    Lê o dump, filtra pelos meses alvo e salva JSONs mensais.
    """
    ...
    # Tenta detectar encoding lendo pouco
    enc = "utf-8"
    try:
        # Pega amostra
    ...
    # Buffer em memória para os meses alvo: data[ym][key] = Agg
    # Se o dump for gigante, isso pode crescer.
    # Otimização: processar e limpar? Não, pandas chunksize resolve a leitura.
    # A agregação final (alguns MBs de JSON) cabe na RAM.
    ...
    for chunk in reader:
        ...
        if first:
            ...
            if not cols['name'] or not cols['date']:
                print("CG: CRÍTICO - Colunas obrigatórias (Nome/Data) não encontradas.")
                return
            first = False

        # Prepara dados
        # 1. Parse Data -> YYYY-MM
        # Tenta formato DD/MM/YYYY
        dates = chunk[cols['date']].fillna("")
        # Regex rápido para capturar YYYY e MM de DD/MM/YYYY ou YYYY-MM-DD
        # Assume DD/MM/YYYY prioritário
        ...
        # 2. Filtra Janela (Apenas meses alvo)
        # Isso descarta 90% do lixo se o dump for antigo
        ...
        # Normaliza Resolução (S/N -> 1/0)
        if cols['resolved']:
            # Assume 'S', 'Sim', 'Resolvida' como 1
        ...
        for (ym, key, display_name), g in grp:
            ...
            # CNPJ (se tiver na coluna e ainda não tiver no agg)
    ...
    for ym, data_map in monthly_data.items():
        ...
        # Converte para formato raw esperado pelo orquestrador
        by_name_raw = {}
        by_cnpj_raw = {}
        
        for k, agg in data_map.items():
            # Recalcula médias para o raw stats
            avg = 0.0
...
def sync_monthly_cache_from_dump_if_needed(target_yms: List[str], monthly_dir: str):
    """Entrypoint para o build_consumidor_gov.py"""
    # Verifica se faltam muitos meses

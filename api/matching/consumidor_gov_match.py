...
def _soft_overlap_weight(q_tokens: set[str], t_tokens: set[str]) -> float:
    ...
    for qt in q_tokens:
        ...
        if qt in t_map:
            ...
            for tt in t_map:
                if qt in GENERIC_TERMS or tt in GENERIC_TERMS:
                    continue
...
class NameMatcher:
        ...
        if trade_name and trade_name.strip():
            ...
            if t.lower() != q_name.lower() and not is_likely_b2b(t):
                ...
                if score_t > score_l:
                    if m_trade:
                        m_trade = MatchMeta(
                            f"{m_trade.method}_trade",
                            m_trade.score,
                            t,
                            m_trade.matched_name,
                            m_trade.matched_cnpj,
                            m_trade.is_b2b
                        )
    ...
    def _match_text(
        self, query: str, threshold: float, seq_threshold: float
    ) -> tuple[dict[str, Any] | None, MatchMeta]:

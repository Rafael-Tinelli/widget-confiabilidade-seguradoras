...
def _calculate_buhlmann_credibility(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Em produção real, usaríamos EPV/VHM temporal.
    ...
    def get_adj_lr(row):
        P = row['premiums']
        if P <= 0: return row['loss_ratio'] # Sem prêmio, mantém observado
    ...
    # Curva Gaussiana: O ideal é LR=0.60 (60%). Muito abaixo é atrito, muito acima é risco.
    # Score = 100 * exp( - (LR - 0.6)^2 / 2*sigma^2 )
...
def _calculate_power_law_complaints(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Somamos 1 ao prêmio para evitar div/0
    ...
    # Rank 1 (menor reclamação) -> Score 100
...
def _calculate_bayes_reputation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if pd.isna(global_mean): global_mean = 3.0
    
    C = PRIOR_C
    
    # Nota Bayesiana
    # Numerador: (C * MediaGlobal) + (NumReviews * MediaObservada)
    # Denominador: C + NumReviews
    df['bayes_rating'] = (
        (C * global_mean) + 
...
def _calculate_product_density(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcula score de produto baseado na densidade (placeholder para lógica OPIN).
    Futuramente, lerá o JSON de produtos.
    """
    # Por enquanto, usamos uma heurística simples se não houver dados de produto
    # Se a seguradora tem flag 'openInsuranceParticipant', ganha bônus
    df['score_product'] = np.where(df['is_opin'], 80, 40)
    
    # Adiciona variação aleatória baseada no tamanho (proxy para portfólio)
    # EM PRODUÇÃO: Substituir pela contagem real de coberturas dos JSONs
    return df

def _calculate_friction(df: pd.DataFrame) -> pd.DataFrame:
    """
    Calcula índice de atrito (LAE/Claims).
    Como ainda não temos LAE no parser, usamos placeholder neutro.
    """
    # Se LAE não existe, assume atrito médio (score 80)
...
def run_scoring_pipeline(insurers_list: list[dict]) -> list[dict]:
    ...
    # 1. Converte Lista de Dicts para DataFrame Pandas (Flat)
    rows = []
    ...
    # 3. Agregação Final
    # Reputação Combine: 60% Volume Ajustado + 40% Nota Qualitativa
    ...
    # 4. Reconstrói a Lista Original com os Scores

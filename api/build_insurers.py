...
def main() -> None:
    ...
    for raw_cnpj, comp in companies.items():
        ...
        # B. Estrutura de Reputação (CORREÇÃO: Null Handling)
        ...
        if rep_entry:
            ...
            # Se faltar dado essencial, não penaliza com 0.0, marca como None
            if sat_avg is None and res_rate is None:
                reputation_data = None
            else:
                reputation_data = {
                    ...
                    "metrics": {
                        "satisfaction_avg": float(sat_avg) if sat_avg is not None else None,
                        "resolution_rate": float(res_rate) if res_rate is not None else None,

...
def build_full_universe_provider_index(entities: list[dict[str, Any]]) -> dict[str, Any]:
    """Build deterministic exact/core indexes across the whole classified universe.

    This is intentionally broader than the 157-insurer matcher. It allows the
    Consumer.gov evidence layer to distinguish a current ordinary insurer from
    a known non-comparable entity (capitalization, open pension, Sandbox,
    reinsurance etc.) instead of leaving every non-157 label as generic
    `unresolved`.

    No fuzzy matching is used. A key is usable only when it identifies one
    canonical entity across the complete inventory.
    """

...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the v2 liquidity experiment.")
    parser.add_argument(
        "--eligibility-input",
        type=Path,
        help=(
            "Use an already materialized eligibility inventory. Gate 4 passes this "
            "explicitly so liquidity never rebuilds live regulatory identity sources."
        ),
    )
    parser.add_argument(
        "--ses-zip",
        ...
        help="Validated BaseCompleta.zip snapshot used for liquidity derivation.",
    )
...
def main() -> None:
    ...
    path = write_liquidity_experiment(payload, args.output)
